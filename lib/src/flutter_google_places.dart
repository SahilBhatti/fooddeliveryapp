...
class _PlacesAutocompleteOverlayState extends PlacesAutocompleteState {
            ...
            child: ListBody(
              children: _response!.predictions
                  .map(
                    (p) => PredictionTile(
                      prediction: p,
                      onTap: address(_response),
                      // onTap: Navigator.of(context).pop,
                      // onTap: _addtoCart(context, _response),
                    ),
                  )
                  .toList(),
            ),
  ...
  address(var location){
    ...
    //   context: context,
    //   apiKey: "AIzaSyDLdAEq-U9bDQdIkPGl9AXCELyly7Q9EnQ",
    //   onError: onError,
    //   // mode: _mode,
    //   language: "fr",
    ...
    // displayPrediction(p, context);
      ...
      displayPrediction(Prediction ?p, BuildContext context) async {
  ...
  Widget build(BuildContext context) {
    return ListView(
      children: predictions
          .map((Prediction p) => PredictionTile(prediction: p, onTap: address(p.reference)))
          .toList(),
    );
  }
}
address(var locations){
  print(locations);
}
